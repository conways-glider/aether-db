...
impl Database {
    ...
    pub fn insert(&self, key: String, value: Value) {
        ...
        if should_notify {
            self.expiry_notification.notify_one();
        }
    }

    pub fn get(&self, key: &str) -> Option<Value> {
        self.db.get(key).map(|entry| entry.value().clone())
    }

    fn next_expiration(&self) -> Option<OffsetDateTime> {
    ...
    fn remove_expired_values(&self) -> Option<Duration> {
        ...
        // TODO: This could be more efficient by caching expirations
        self.next_expiration().map(|expiration| {
...
pub async fn remove_expired_entries(database: Arc<Database>) {
    loop {
        if let Some(instant) = database.remove_expired_values() {
            tokio::select! {
...
mod tests {
    ...
    async fn test_database_set_expiration() {
        ...
        // Start up expiry handling
        let database = Arc::new(Database::default());
        tokio::spawn(remove_expired_entries(database.clone()));
